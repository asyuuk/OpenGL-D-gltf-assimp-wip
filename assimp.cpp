...
Assimp3D::Assimp3D(Object* Owner, Drawing::Renderer* renderer)
{
	shader = renderer;
	if (!Loadassimp())
	{
		std::cout << "Loadassimp()Error" << std::endl;
	}
}
...
void Assimp3D::Draw3D(Gltf3D* gltf,float x, float y, float z, float size, float angle,glm::vec3 center, float cameraposx, float cameraposy, float cameraposz)
{
...
GLuint Assimp3D::LoadImage(const std::string& filename)
{
